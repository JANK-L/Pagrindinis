import React from "react";
import "./Pirkimas.css";

const Pirkimas = ({ kaina }) => {
  return (
    <div>
      <p className="kaina">Kaina:</p>
      <h1>{kaina} &euro;</h1>

      <button className="pirkti">
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <g clipPath="url(#clip0_326_186)">
            <path
              d="M23.9375 0.0625V23.9375H0.0625V0.0625H23.9375Z"
              stroke="#252525"
              strokeWidth="0.125"
            />
            <path
              d="M19.6561 6.75H4.34298C4.15863 6.74998 3.98063 6.81734 3.8425 6.93942C3.70436 7.0615 3.61562 7.22986 3.59298 7.41281L2.25704 18.6628C2.24468 18.7684 2.2549 18.8754 2.28702 18.9767C2.31914 19.078 2.37242 19.1713 2.44333 19.2505C2.51425 19.3296 2.60118 19.3928 2.69836 19.4358C2.79555 19.4788 2.90076 19.5007 3.00704 19.5H20.992C21.0983 19.5007 21.2035 19.4788 21.3007 19.4358C21.3979 19.3928 21.4848 19.3296 21.5557 19.2505C21.6267 19.1713 21.6799 19.078 21.7121 18.9767C21.7442 18.8754 21.7544 18.7684 21.742 18.6628L20.4061 7.41281C20.3835 7.22986 20.2947 7.0615 20.1566 6.93942C20.0184 6.81734 19.8405 6.74998 19.6561 6.75Z"
              stroke="#252525"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
            <path
              d="M8.25 9.75V6C8.25 5.00544 8.64509 4.05161 9.34835 3.34835C10.0516 2.64509 11.0054 2.25 12 2.25C12.9946 2.25 13.9484 2.64509 14.6517 3.34835C15.3549 4.05161 15.75 5.00544 15.75 6V9.75"
              stroke="#252525"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
            />
          </g>
          <defs>
            <clipPath id="clip0_326_186">
              <rect width="24" height="24" fill="white" />
            </clipPath>
          </defs>
        </svg>
        Add to cart
      </button>
    </div>
  );
};

export default Pirkimas;
